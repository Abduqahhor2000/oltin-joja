...
const BaseInput = (props) => {
    ...
    return (
        <>
            <div className={`inputBox  relative w-full h-[45px] ${item_name === "Product name" ? "col-span-2" : item_name === "Price" ? "col-span-1" : item_name === "Discount" ? "col-span-1" : null}`}>
                <input
                    key={id}
                    autoComplete="off"
                    {...inputProps}
                    onKeyDown={inputProps.type == "number" ? (evt) => evt.key === 'e' && evt.preventDefault() : null}
                    onChange={onChange}
                    required
                    className="peer p-3 w-full h-full border outline-none  valid:border-green-300 rounded-lg focus:border-Primary/03 " />
                <span className="text-sm px-2 rounded-lg absolute top-[12px] left-3  pointer-events-none  peer-valid:translate-y-[-20px] peer-focus:translate-y-[-20px] peer-focus:translate-x-[-8px]  peer-valid:translate-x-[-8px] peer-valid:text-[13px] peer-focus:text-[13px]  ease-linear duration-[0.2s] text-[#9A9A9A] bg-white">{item_name}</span>
